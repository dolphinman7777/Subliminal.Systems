...
export function AudioModule({
  ...
  const mixAudio = async (affirmationAudioUrl: string, backingTrackUrl: string) => {
    setIsLoading(true);
    try {
      if (!affirmationAudioUrl || !backingTrackUrl) {
        throw new Error('Missing audio URLs');
      }

      const normalizedTtsVolume = ttsVolume / 100;
      const normalizedBackingVolume = backingTrackVolume / 100;

      console.log('Mixing audio with params:', {
        ttsAudioUrl: affirmationAudioUrl,
        backingTrack: backingTrackUrl,
        ttsVolume: normalizedTtsVolume,
        backingVolume: normalizedBackingVolume
      });
